...
### Reverse priority mapping, class [5, 4, 3, 2, 1, 0] if none ### then class 6 default. class 0 is normal (most common).  
...
def create_ptbxl_labels():
    ...
    def map_dx_to_class(dx_codes):
        """Map Dx codes to class labels based on SNOMED mapping with priority order"""
        if pd.isna(dx_codes):
        ...
        # Define search priority order: 5, 4, 3, 2, 1, 0, then 6 (other)
        priority_order = [5, 4, 3, 2, 1, 0]
        
        # Create reverse mapping from class to SNOMED code
        class_to_snomed = {v: k for k, v in snomed_to_class.items()}
        
        # Search in priority order
        for priority_class in priority_order:
            if priority_class in class_to_snomed:
                target_snomed = class_to_snomed[priority_class]
                if target_snomed in codes:
                    return priority_class
        
        # If no priority match found, assign to "Other arrhythmias" class
        return 6
